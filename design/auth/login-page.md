...
The primary login button uses the highlighted button variant and normal Button typography with taller auth-only height; auth pages may make the button full width and taller locally.
...
- Do use taller auth-only `h-12` button height for the primary auth submit.
- Don't copy Laravel/Blade/Livewire implementation details into this design system.

## Implementation Notes

The auth submit button style is shared with Register and TOTP challenge pages. Apply auth-local classes such as `h-12 w-full justify-center px-4` on the highlighted Button rather than changing all highlighted buttons globally.
...
- [ ] Primary Login button is highlighted and uses taller auth Button height with normal Button typography.

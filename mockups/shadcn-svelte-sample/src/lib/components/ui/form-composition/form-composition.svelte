...
<form class="space-y-4 rounded-sm border border-neutral-200 bg-white p-4 dark:border-coolgray-300 dark:bg-coolgray-100" onsubmit={(e) => e.preventDefault()}>
  ...
  <FormField forId="name" label="Name" required helper="Used in resource lists and URLs." error={nameError || undefined}>
    <Input id="name" class="dark:bg-base" bind:value={name} dirty={name.length > 0} aria-invalid={!!nameError} placeholder="production-api" />
  </FormField>
  <FormField forId="domain" label="Domain" helper="Public hostname for this resource." error={domainError || undefined}>
    <Input id="domain" class="dark:bg-base" bind:value={domain} dirty={domain !== "api.example.com"} aria-invalid={!!domainError} />
  </FormField>
  <FormField forId="email" label="Notification email" required helper="Receives deployment and validation alerts." error={emailError || undefined}>
    <Input id="email" class="dark:bg-base" type="email" bind:value={email} dirty={email !== "ops@example.com"} aria-invalid={!!emailError} />
  </FormField>
  <FormField forId="phone" label="Escalation phone" helper="Optional international number for urgent alerts." error={phoneError || undefined}>
    <Input id="phone" class="dark:bg-base" type="tel" bind:value={phone} dirty={phone !== "+1 555 123 4567"} aria-invalid={!!phoneError} />
  </FormField>

...
<div data-slot="theme-switcher" class={cn("w-full", collapsed && "w-8")}>
  {#if collapsed}
    <button
      type="button"
      class="flex size-8 items-center justify-center rounded-sm hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
    ...
    >
      ...
      {:else if selectedTheme === "system"}
        <svg class="size-4 shrink-0 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="4" y="5" width="16" height="11" rx="1.5"/><path d="M9 20h6M12 16v4"/></svg>
      {:else}
        <svg class="size-4 shrink-0 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>
      {/if}
    </button>
  {:else}
    <div class="flex min-h-7 w-full items-center gap-3 rounded-sm px-2 py-1 text-sm hover:bg-muted hover:text-foreground">
      <span class="shrink-0">Theme</span>
      <div class="ml-auto inline-grid grid-cols-3 rounded-sm border border-border bg-muted p-0.5" role="radiogroup" aria-label="Theme preference">
        {#each themeOptions as option}
          <button
            ...
            class={cn("grid size-6 place-items-center rounded-sm text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring", selectedTheme === option.value && "bg-primary text-primary-foreground shadow-sm")}
